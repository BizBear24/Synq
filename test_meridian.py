...
class MeridianTests(unittest.TestCase):
 ...
 def test_queue_and_rerun(self):
  expected=len(json.loads((self.tmp/'tickets.json').read_text()))
  a=self.ops.run(); b=self.ops.run(); self.assertEqual(a['input_records'],expected); self.assertGreater(a['quarantined'],0); self.assertEqual(b['new_actions'],0)
 ...
 def test_beyond_fifty_needs_review(self):
  payload=[{'ticket_id':'TKT-TEST-50','created_at':'2026-08-22T10:00:00','vehicle':'CH81AQ4130','origin_hub':'Chandigarh','km_from_origin_hub':51,'destination':'Delhi','issue':'tyre','client':'Internal'}]
  (self.tmp/'beyond_50.json').write_text(json.dumps(payload)); self.ops.run('beyond_50.json'); t=self.ops.get_ticket('TKT-TEST-50'); self.assertEqual(t['status'],'NEEDS_REVIEW'); self.assertIn('Nearest-hub distance',t['decision']['review'])
